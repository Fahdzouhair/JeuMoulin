...
bool GameScene::init()
{
	...
	auto background = Sprite::create("Board_fahd.png");
	...
	auto goBack = cocos2d::ui::Button::create("goBack.png", "goBack_selected.png");
	goBack->setPosition(Point (65 + origin.x, (visibleSize.height / 1.07 + origin.y)-10));
	goBack->addTouchEventListener([&](Ref * sender, cocos2d::ui::Widget::TouchEventType type) {
		switch (type)
		{
	...
	goBack->setScale(rY*0.9);
	this->addChild(goBack);
	
	auto music = cocos2d::ui::Button::create("Music.png", "Music_selected.png");
	music->setPosition(Point(visibleSize.width / 1.06 + origin.x, (visibleSize.height / 1.08 + origin.y)+10));
	music->addTouchEventListener([&](Ref* sender, cocos2d::ui::Widget::TouchEventType type) {
		switch (type)
		{
	...
	music->setScale(rY * 0.9);
	this->addChild(music);

	auto pause = cocos2d::ui::Button::create("pause.png", "pause_selected.png");
	pause->setPosition(Point(visibleSize.width / 1.06 + origin.x, music->getPosition().y - music->getBoundingBox().size.height * 1.2f));
	pause->addTouchEventListener([&](Ref* sender, cocos2d::ui::Widget::TouchEventType type) {
		switch (type)
		{
	...
	pause->setScale(rY * 0.9);
	this->addChild(pause);

	auto exit = cocos2d::ui::Button::create("exit.png", "exit_selected.png");
	exit->setPosition(Point(visibleSize.width / 1.06 + origin.x, pause->getPosition().y - pause->getBoundingBox().size.height * 1.2f));
	...
	exit->setScale(rY * 0.9);
	this->addChild(exit);
	

	for (int i = 0; i < 24; i++)
	{
		tokens[i] = Sprite::create("sp.png");
	}
	...
	black[0] = Sprite::create("b.png");
	black[0]->setPosition(Point(visibleSize.width / 5.2+ origin.x, visibleSize.height / 2.20 + origin.y));//done 23
	this->addChild(black[0], -1);

	black[1] = Sprite::create("b.png");
	black[1]->setPosition(Point(visibleSize.width / 5.2+ origin.x- black[1]-> getContentSize().width, visibleSize.height / 2.20 + origin.y));//done 23
	this->addChild(black[1], -1);

	black[2] = Sprite::create("b.png");
	black[2]->setPosition(Point(visibleSize.width / 5.2 + origin.x - black[1]->getContentSize().width*2, visibleSize.height / 2.20 + origin.y));//done 23
	this->addChild(black[2], -1);
	///////
	black[3] = Sprite::create("b.png");
	black[3]->setPosition(Point(visibleSize.width / 5.2 + origin.x, visibleSize.height / 2.20 + origin.y - black[1]->getContentSize().width));//done 23
	this->addChild(black[3], -1);

	black[4] = Sprite::create("b.png");
	black[4]->setPosition(Point(visibleSize.width / 5.2 + origin.x - black[1]->getContentSize().width, visibleSize.height / 2.20 + origin.y - black[1]->getContentSize().width));//done 23
	this->addChild(black[4], -1);

	black[5] = Sprite::create("b.png");
	black[5]->setPosition(Point(visibleSize.width / 5.2 + origin.x - black[1]->getContentSize().width * 2, visibleSize.height / 2.20 + origin.y - black[1]->getContentSize().width));//done 23
	this->addChild(black[5], -1);
	///////////
	black[6] = Sprite::create("b.png");
	black[6]->setPosition(Point(visibleSize.width / 5.2 + origin.x, visibleSize.height / 2.20 + origin.y - black[1]->getContentSize().width*2));//done 23
	this->addChild(black[6], -1);

	black[7] = Sprite::create("b.png");
	black[7]->setPosition(Point(visibleSize.width / 5.2 + origin.x - black[1]->getContentSize().width, visibleSize.height / 2.20 + origin.y - black[1]->getContentSize().width*2));//done 23
	this->addChild(black[7], -1);

	black[8] = Sprite::create("b.png");
	black[8]->setPosition(Point(visibleSize.width / 5.2 + origin.x - black[1]->getContentSize().width * 2, visibleSize.height / 2.20 + origin.y - black[1]->getContentSize().width*2));//done 23
	this->addChild(black[8], -1);

	///////////////////////////////////////////////////////////////////////////////////white


	white[0] = Sprite::create("w.png");
	white[0]->setPosition(Point(visibleSize.width / 1.06 + origin.x, visibleSize.height / 2.20 + origin.y));//done 23
	this->addChild(white[0], -1);

	white[1] = Sprite::create("w.png");
	white[1]->setPosition(Point(visibleSize.width / 1.06 + origin.x - white[1]->getContentSize().width, visibleSize.height / 2.20 + origin.y));//done 23
	this->addChild(white[1], -1);

	white[2] = Sprite::create("w.png");
	white[2]->setPosition(Point(visibleSize.width / 1.06 + origin.x - white[1]->getContentSize().width * 2, visibleSize.height / 2.20 + origin.y));//done 23
	this->addChild(white[2], -1);
	///////
	white[3] = Sprite::create("w.png");
	white[3]->setPosition(Point(visibleSize.width / 1.06 + origin.x, visibleSize.height / 2.20 + origin.y - white[1]->getContentSize().width));//done 23
	this->addChild(white[3], -1);

	white[4] = Sprite::create("w.png");
	white[4]->setPosition(Point(visibleSize.width / 1.06 + origin.x - white[1]->getContentSize().width, visibleSize.height / 2.20 + origin.y - white[1]->getContentSize().width));//done 23
	this->addChild(white[4], -1);

	white[5] = Sprite::create("w.png");
	white[5]->setPosition(Point(visibleSize.width / 1.06 + origin.x - white[1]->getContentSize().width * 2, visibleSize.height / 2.20 + origin.y - white[1]->getContentSize().width));//done 23
	this->addChild(white[5], -1);
	///////////
	white[6] = Sprite::create("w.png");
	white[6]->setPosition(Point(visibleSize.width / 1.06 + origin.x, visibleSize.height / 2.20 + origin.y - white[1]->getContentSize().width * 2));//done 23
	this->addChild(white[6], -1);

	white[7] = Sprite::create("w.png");
	white[7]->setPosition(Point(visibleSize.width / 1.06 + origin.x - white[1]->getContentSize().width, visibleSize.height / 2.20 + origin.y - white[1]->getContentSize().width * 2));//done 23
	this->addChild(white[7], -1);

	white[8] = Sprite::create("w.png");
	white[8]->setPosition(Point(visibleSize.width / 1.06 + origin.x - white[1]->getContentSize().width * 2, visibleSize.height / 2.20 + origin.y - white[1]->getContentSize().width * 2));//done 23
	this->addChild(white[8], -1);

	
	
	moulin = Sprite::create("Hard.png");
	moulin->setPosition(Point(visibleSize.width / 4 + origin.x, visibleSize.height / 4 + origin.y));
	this->addChild(moulin);
...
void GameScene::onTouchEnded(cocos2d::Touch* touch, cocos2d::Event* event)
{
	...
	// if we got a moulin we gonna set a position for the token that we took 
				...
				{																			// phase 3 
					if (database[i] == 0 && (validmove(source, destination) ||(noir==3 &&  player==-1)||(blanc==3&&player==1)) && tokens[i]->getBoundingBox().containsPoint(touchPoint))
					{
						...
						if (player == -1)
						{
							...
							black[selected]->setTexture("b.png");
						}
						if (player == 1)
						{
							white[selected]->runAction(moveTo);
							white[selected]->setTexture("w.png");
						}
						database[source] = 0;
			...
			for (int i = 0; i < 9; i++)
			{
				if (player == -1)
					if (black[i] != 0 && black[i]->getBoundingBox().containsPoint(touchPoint))
					{
						for (int k = 0; k < 9; k++)
						{
							black[k]->setTexture("b.png");
						}
						selected = i;
						black[selected]->setTexture("big_b.png");
						hold = 1;
					}
				if (player == 1)
					if (white[i] != 0 && white[i]->getBoundingBox().containsPoint(touchPoint))
					{
						for (int k = 0; k < 9; k++)
						{
							white[k]->setTexture("w.png");
						}
						selected = i;
						white[selected]->setTexture("big_w.png");
						hold = 1;
					}
			}
			if(hold)
